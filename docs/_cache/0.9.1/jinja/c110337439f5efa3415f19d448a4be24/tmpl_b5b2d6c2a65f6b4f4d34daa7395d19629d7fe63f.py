...
def root(context, missing=missing):
    ...
    def macro():
        ...
        return concat(t_3)
    caller = Macro(environment, macro, None, (), False, False, False, context.eval_ctx.autoescape)
    yield context.call(environment.extensions['strictdoc.export.html.html_templates.AssertExtension']._assert, t_1(environment.getattr((undefined(name='custom_metadata_row_context') if l_0_custom_metadata_row_context is missing else l_0_custom_metadata_row_context), 'field')), 'row_with_metadata: field must be defined.', caller=caller)
    if parent_template is None:
        yield '\n'
    def macro():
        t_4 = []
