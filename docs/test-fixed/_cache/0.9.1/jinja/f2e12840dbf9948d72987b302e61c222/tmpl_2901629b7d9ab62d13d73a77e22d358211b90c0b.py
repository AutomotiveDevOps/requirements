...
def root(context, missing=missing):
    ...
    for l_1_change, l_1_loop in LoopContext(environment.getattr(environment.getattr(environment.getattr((undefined(name='view_object') if l_0_view_object is missing else l_0_view_object), 'change_container'), 'change_stats'), 'changes'), undefined):
        ...
        if ((environment.getattr(l_1_change, 'change_type') == 'Requirement added') or (environment.getattr(l_1_change, 'change_type') == 'Section added')):
            ...
            yield context.call(environment.extensions['strictdoc.export.html.html_templates.AssertExtension']._assert, (not t_1(environment.getattr(l_1_change, 'lhs_section'))), l_1_change, caller=caller, _loop_vars=_loop_vars)
            yield '\n          '
            ...
            def macro():
                ...
                return concat(t_3)
            caller = Macro(environment, macro, None, (), False, False, False, context.eval_ctx.autoescape)
            yield context.call(environment.extensions['strictdoc.export.html.html_templates.AssertExtension']._assert, 0, 'Must not reach here.', caller=caller, _loop_vars=_loop_vars)
            yield '\n        '
        yield '\n      </div>\n      <div class="changelog_change_node">\n        '
