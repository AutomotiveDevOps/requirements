...
def root(context, missing=missing):
    ...
    for l_1_document, l_1_loop in LoopContext(context.call(environment.getattr((undefined(name='view_object') if l_0_view_object is missing else l_0_view_object), 'iterate_documents')), undefined):
        ...
        for (l_2_section_or_requirement, l_2__) in context.call(environment.getattr((undefined(name='document_iterator') if l_1_document_iterator is missing else l_1_document_iterator), 'all_content'), print_fragments=True, print_fragments_from_files=False, _loop_vars=_loop_vars):
