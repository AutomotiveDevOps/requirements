...
def root(context, missing=missing):
    ...
    yield context.call(environment.extensions['strictdoc.export.html.html_templates.AssertExtension']._assert, t_1((undefined(name='comment_field_row_context') if l_0_comment_field_row_context is missing else l_0_comment_field_row_context)), 'row_with_comment: row_context must be defined.', caller=caller)
    def macro():
        t_3 = []
        pass
        return concat(t_3)
    caller = Macro(environment, macro, None, (), False, False, False, context.eval_ctx.autoescape)
    yield context.call(environment.extensions['strictdoc.export.html.html_templates.AssertExtension']._assert, t_1(environment.getattr((undefined(name='comment_field_row_context') if l_0_comment_field_row_context is missing else l_0_comment_field_row_context), 'errors')), 'row_with_comment: errors must be defined.', caller=caller)
    def macro():
        t_4 = []
        pass
        return concat(t_4)
