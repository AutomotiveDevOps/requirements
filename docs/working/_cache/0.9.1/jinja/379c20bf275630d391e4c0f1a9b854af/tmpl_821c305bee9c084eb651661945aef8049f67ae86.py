...
def root(context, missing=missing):
    ...
    yield context.call(environment.extensions['strictdoc.export.html.html_templates.AssertExtension']._assert, (environment.getattr(environment.getattr((undefined(name='requirement') if l_0_requirement is missing else l_0_requirement), '__class__'), '__name__') == 'SDocNode'), 'Expected requirement', caller=caller)
    def macro():
        t_3 = []
        pass
        return concat(t_3)
    caller = Macro(environment, macro, None, (), False, False, False, context.eval_ctx.autoescape)
    yield context.call(environment.extensions['strictdoc.export.html.html_templates.AssertExtension']._assert, (environment.getattr(environment.getattr(environment.getattr((undefined(name='view_object') if l_0_view_object is missing else l_0_view_object), 'link_renderer'), '__class__'), '__name__') == 'LinkRenderer'), 'Expected link renderer', caller=caller)
    l_0_ranged = False
    ...
    if (undefined(name='requirement_file_links') if l_0_requirement_file_links is missing else l_0_requirement_file_links):
        ...
        for (l_1_link, l_1_markers) in (undefined(name='requirement_file_links') if l_0_requirement_file_links is missing else l_0_requirement_file_links):
            ...
            def macro():
                ...
                return concat(t_4)
            caller = Macro(environment, macro, None, (), False, False, False, context.eval_ctx.autoescape)
            yield context.call(environment.extensions['strictdoc.export.html.html_templates.AssertExtension']._assert, (environment.getattr(environment.getattr(l_1_link, '__class__'), '__name__') == 'str'), 'Expected str', caller=caller, _loop_vars=_loop_vars)
